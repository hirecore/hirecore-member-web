...
/**
 * Determines if the current auth is macOS
 * @returns boolean indicating if the current auth is Mac
 */
...
/**
 * Formats a shortcut key based on the auth (Mac or non-Mac)
 * @param key - The key to format (e.g., "ctrl", "alt", "shift")
 * @param isMac - Boolean indicating if the auth is Mac
...
/**
 * 브라우저 Canvas API로 이미지 파일을 WebP로 변환한다.
 *
 * - 이미 WebP인 파일은 변환 없이 즉시 반환 (이중 변환 방지)
 * - SVG / GIF는 래스터화 이슈로 원본 반환
 * - canvas.toBlob('image/webp') 미지원 환경(구형 Safari)에서는 원본 반환
 *
...
export async function toWebP(file: File, quality = 0.85): Promise<File> {
  ...
  try {
    ...
    return await new Promise<File>((resolve) => {
      canvas.toBlob(
        (blob) => {
          if (!blob) { resolve(file); return } // 변환 실패 → 원본 반환
    ...
    return file // createImageBitmap 실패 등 예외 → 원본 반환
  }
}
...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `파일 크기가 최대 용량(${MAX_FILE_SIZE / (1024 * 1024)}MB)을 초과했습니다`
    )
  }
