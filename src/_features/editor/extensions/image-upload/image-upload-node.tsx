...
import { CloseIcon } from "@/_features/editor/ui/icons/close-icon"
import "@/_features/editor/extensions/image-upload/image-upload-node.scss"
import { focusNextNode, isValidPosition } from "@/_features/editor/lib/tiptap-helpers"
...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    if (file.size > options.maxSize) {
      const error = new Error(
        `파일 크기가 최대 용량(${options.maxSize / 1024 / 1024}MB)을 초과했습니다`
      )
    ...
    try {
      if (!options.upload) {
        throw new Error("업로드 함수가 설정되지 않았습니다")
      }
      ...
      if (!url) throw new Error("업로드에 실패했습니다")
  ...
  const uploadFiles = async (files: File[]): Promise<string[]> => {
    ...
    if (options.limit && files.length > options.limit) {
      options.onError?.(
        new Error(
          `한 번에 최대 ${options.limit}개의 파일만 업로드할 수 있습니다`
        )
      )
      return []
    }
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>클릭하여 업로드</em> 또는 드래그하여 놓기
      </span>
      <span className="tiptap-image-upload-subtext">
        최대 {limit}개, 각 {maxSize / 1024 / 1024}MB 이하
      </span>
    </div>
  </>
)
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        ...
        if (urls.length === 1) {
          ...
          chain
            .insertContentAt(pos, {
              type: extension.options.type as string,
              attrs: {
                ...extension.options,
                src: urls[0],
                alt: filename,
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0) {
      extension.options.onError?.(new Error("파일을 선택하지 않았습니다"))
      return
    }
